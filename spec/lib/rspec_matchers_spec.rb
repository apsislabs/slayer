...
RSpec.describe 'Custom Rspec Matchers' do
  ...
  it 'be_ok_result' do
    expect(WhateverCommand.call(succeed: true, status: :foo, message:"foo", value: "foo")).to be_ok_result.with_status(:foo).with_value("foo").with_message("foo")
    expect(WhateverCommand.call(succeed: true, status: :foo, message:"foo", value: "foo")).to be_ok_result.with({ status: :foo, value: "foo", message: "foo" })
  end

  it 'be_err_result' do
    expect(WhateverCommand.call(succeed: false, status: :foo, message:"foo", value: "foo")).to be_err_result.with_status(:foo).with_value("foo").with_message("foo")
    expect(WhateverCommand.call(succeed: false, status: :foo, message:"foo", value: "foo")).to be_err_result.with({ status: :foo, value: "foo", message: "foo" })
  end
