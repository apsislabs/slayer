...
module Minitest::Assertions
  def assert_executes(exactly: nil,
                      at_least: nil,
                      message: 'The given block did not execute the desired statement', &block)

    assert_proc = proc { |a, m| assert(a, m) }
  ...
  def refute_executes(exactly: nil,
                      at_least: nil,
                      message: 'The given block executed the statement it should '\
                               'not have executed', &block)

    refute_proc = proc { |a, m| refute(a, m) }
